...
import type { SavingsCircleTransaction } from '@/features/savings-circles/types';
import {
	circleTransactionKindLabel,
	formatDisplayDate,
	isCircleOutflow,
...
interface SavingsCircleHistoryListProps {
	items: SavingsCircleTransaction[];
	preferredCurrency: string;
	onEdit: (item: SavingsCircleTransaction) => void;
	onDelete: (item: SavingsCircleTransaction) => void;
}

const ROW_GRID = 'sm:grid-cols-[8.5rem_minmax(0,1fr)_minmax(0,1.2fr)_7rem_9rem_2.5rem]';

export function SavingsCircleHistoryList({
	items,
	preferredCurrency,
	onEdit,
	onDelete,
}: SavingsCircleHistoryListProps) {
	return (
		<div
			className="overflow-hidden rounded-xl border border-border bg-card shadow-xs"
			data-testid="circle-history-list"
		>
			<div
				className={cn(
			...
			<ul className="divide-y divide-border">
				{items.map((item) => {
					const isOutflow = isCircleOutflow(item);
					const KindIcon = isOutflow ? TrendingDown : TrendingUp;
					const kind = circleTransactionKindLabel(item);
					...
					return (
						<li
							...
							data-testid={`circle-row-${item.id}`}
						>
								...
								<DropdownMenu>
									<DropdownMenuTrigger asChild>
										<Button
											...
											data-testid={`circle-entry-actions-${item.id}`}
										>
