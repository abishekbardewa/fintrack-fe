...
import {
	formatDisplayDate,
	investmentTransactionKindLabel,
	isInvestmentOutflow,
	transactionDisplayAmount,
} from '@/features/investments/utils';
import { cn } from '@/lib/utils';

interface InvestmentHistoryListProps {
	items: InvestmentTransaction[];
	preferredCurrency: string;
	canEdit: boolean;
	onEdit: (item: InvestmentTransaction) => void;
	onDelete: (item: InvestmentTransaction) => void;
}

const ROW_GRID = 'sm:grid-cols-[8.5rem_minmax(0,1fr)_minmax(0,1.2fr)_7rem_9rem_2.5rem]';

export function InvestmentHistoryList({
	items,
	preferredCurrency,
	canEdit,
	onEdit,
	onDelete,
}: InvestmentHistoryListProps) {
	return (
		<div
			className="overflow-hidden rounded-xl border border-border bg-card shadow-xs"
			data-testid="investment-history-list"
		>
			<div
				className={cn(
			...
			<ul className="divide-y divide-border">
				{items.map((item) => {
					const isOutflow = isInvestmentOutflow(item);
					const KindIcon = isOutflow ? TrendingDown : TrendingUp;
					const kind = investmentTransactionKindLabel(item);
									...
									<DropdownMenu>
										<DropdownMenuTrigger asChild>
											<Button
												...
												data-testid={`investment-entry-actions-${item.id}`}
											>
